...
class GraphConfig(StrictModel):
    ...
    def validate_edge_weights(cls,value):
        if any(not key or weight<0 for key,weight in value.items()): raise ValueError('edge_weights require non-empty relation names and non-negative values')
        return value
...
def load_config(path:Path)->OnlineConfig:
    ...
    for section,prefix in (('applicability','VN_LABOR_APPLICABILITY'),('adjudication','VN_LABOR_ADJUDICATION')):
        ...
        if os.getenv(f'{prefix}_TIMEOUT_SECONDS'):
            configured['timeout_seconds']=float(os.environ[f'{prefix}_TIMEOUT_SECONDS'])
